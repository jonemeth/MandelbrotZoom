...
typedef struct
{
    ...
    // k-defaultPalette tree over RGB space, organized in heap fashion
...
// walks the k-defaultPalette tree to pick the palette entry for a desired color.
...
void GifGetClosestPaletteColor( GifPalette* pPal, int r, int g, int b, int* bestInd, int* bestDiff, int treeRoot )
{
    ...
    // take the appropriate color (r, g, or b) for this node of the k-defaultPalette tree
...
// Builds a palette by creating a balanced k-defaultPalette tree of all pixels in the image
void GifSplitPalette(uint8_t* image, int numPixels, int firstElt, int lastElt, int splitElt, int splitDist, int treeNode, bool buildForDither, GifPalette* pal)
{
    ...
    // and split along that axis. (incidentally, this means this isn't a "proper" k-defaultPalette tree but I don't know what else to call it)
    int splitCom = 1;
...
// Creates a palette by placing all the image pixels in a k-defaultPalette tree and then averaging the blocks at the bottom.
...
void GifWriteBit( GifBitStatus* stat, uint32_t bit )
{
    ...
    stat->byte |= static_cast<uint8_t>(bit);
